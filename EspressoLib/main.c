...
int getPLA(int opt, int argc, char **argv, int option, pPLA *PLA, int out_type);
void delete_arg(int *argc, register char **argv, int num);
void init_runtime(void);
int backward_compatibility_hack(int *argc, char **argv, int *option, int *out_type);
void runtime(void);
void usage(void);
...
{
    int ret, i, j, first, last, strategy, out_type, option;
    pPLA PLA, PLA1;
    pcover F, Fold, Dold;
    pset last1, p;
    cost_t cost;
    ...
    ret = backward_compatibility_hack(&argc, argv, &option, &out_type);
    if (ret != 0)
        return ret;

    /* parse command line options*/
    while ((i = getopt(argc, argv, "D:S:de:o:r:stv:x:h")) != EOF) {
        switch (i) {
    ...
    case 2:
        ...
        ret = getPLA(optind++, argc, argv, option, &PLA, out_type);
        if (ret != 0)
            return ret;
        ret = getPLA(optind++, argc, argv, option, &PLA1, out_type);
        if (ret != 0)
            return ret;
        break;
    case 1:
    ...
    *option = 0;
    ...
    for (i = 1; i < (*argc) - 1; i++) {
        if (strcmp(argv[i], "-out") == 0) {
            ...
            if (pla_types[j].key == 0) {
                fprintf(stderr, "espresso: bad keyword \"%s\" following -out\n", argv[i + 1]);
                return 1;
            }
